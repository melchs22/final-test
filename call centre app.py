    ...
    try:
        ...
        for agent in agents:
            ...
            if not perf_df.empty:
                ...
                if not perf_df.empty:
                    ...
                    table_data = [['Metric', 'Value']] + [
                        ...
                        for metric, value in perf_data.items() if pd.notnull(value)
                    ]
                    ...
                    st.warning(f"No performance data for {agent} in the specified date range.")
                    elements.append(Paragraph("No performance data available for this date range.", normal_style))
                    elements.append(Spacer(1, 12))
            else:
                st.warning(f"No performance data found for {agent}.")
            ...
            if not feedback_df.empty:
                ...
                elements.append(Paragraph("No feedback available.", normal_style))
                elements.append(Spacer(1, 12))
        ...
        if buffer_size == 0:
            st.error("Generated PDF buffer is empty.")
            return None
        st.write(f"DEBUG: Generated PDF buffer size: {buffer_size} bytes")
        return buffer
    except Exception as e:
        st.error(f"Error generating PDF report: {str(e)}")
        st.write(f"DEBUG: Exception in generate_pdf_report: {str(e)}")
        return None
